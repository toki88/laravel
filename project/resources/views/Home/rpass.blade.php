<!DOCTYPE html>
<html>

	<head lang="en">
		<meta charset="UTF-8">
		<title>登录</title>
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no">
		<meta name="format-detection" content="telephone=no">
		<meta name="renderer" content="webkit">
		<meta http-equiv="Cache-Control" content="no-siteapp" />

		<link rel="stylesheet" href="/Home/AmazeUI-2.4.2/assets/css/amazeui.css" />
		<link href="/Home/css/dlstyle.css" rel="stylesheet" type="text/css">
	</head>

	<body>

		<div class="login-boxtitle">
			<a href="home.html"><img alt="logo" src="/Home/images/logobig.png" /></a>
		</div>

		<div class="login-banner">
			<div class="login-main">
				<div class="login-banner-bg"><span></span><img src="/Home/images/big.jpg" /></div>
				<div class="login-box">

							<h3 class="title">你记得密码的话就不会来这里了</h3>

							<div class="clear"></div>
						
						<div class="login-form">
						  <form action='/home/dorpass' method='post'>
							   <div class="user-name">
						   		{{ csrf_field() }}
								<!-- @if(session('msg'))
						   		    <h2 class="m-t-0 m-b-15">{{ session('msg') }}</h2>
						   		@else
						   		    <h2 class="m-t-0 m-b-15">Login</h2>
						   		@endif -->
							    <label for="user"><i class="am-icon-user"></i></label>
							    <input type="text" name="name" id="name" placeholder="邮箱/手机/用户名">
						    </div>
		    			   <!-- <div class="user-email">
		    						<label for="email"><i class="am-icon-envelope-o"></i></label>
		    						<input type='text' name='remail'placeholder="请输入验证码"><button type="button" id='but' disabled="disabled">10</button>
		                     </div><br><hr> -->
						    <!-- 请输入邮箱验证码:<input type='text' name='remail'><button type="button" id='but' disabled="disabled">10</button> -->
					         <div class="user-pass">
							    <label for="password"><i class="am-icon-lock"></i></label>
							    <input type="password" name="password" id="password" placeholder="请输入新密码">
					         </div>
					   <!--   </div>
            			请输入验证码：
            			<div class='row'>
            			    <div class='col-xs-4'>
            			        <input type='text' class='login-control m-b-10' name='mycode'>
            			        <img src="{{ url('/Admin/captch/'.time()) }}" onclick="this.src='{{ url('/Admin/captch') }}/'+Math.random()" style="width:100px;height:25px;"/>
            			   </div>
						</div> -->
							<div class="am-cf">
								<input type="submit"  value="登 录" class="am-btn am-btn-primary am-btn-sm">
							</div>
						</form>
						<div class="partner">		
								<h3>合作账号</h3>
							<div class="am-btn-group">
								<li><a href="#"><i class="am-icon-qq am-icon-sm"></i><span>QQ登录</span></a></li>
								<li><a href="#"><i class="am-icon-weibo am-icon-sm"></i><span>微博登录</span> </a></li>
								<li><a href="#"><i class="am-icon-weixin am-icon-sm"></i><span>微信登录</span> </a></li>
							</div>
						</div>	

				</div>
			</div>
		</div>


					<div class="footer ">
						<div class="footer-hd ">
							<p>
								<a href="/home/home">恒望科技</a>
								<b>|</b>
								<a href="# ">商城首页</a>
								<b>|</b>
								<a href="# ">支付宝</a>
								<b>|</b>
								<a href="# ">物流</a>
							</p>
						</div>
						<div class="footer-bd ">
							<p>
								<a href="# ">关于恒望</a>
								<a href="# ">合作伙伴</a>
								<a href="# ">联系我们</a>
								<a href="# ">网站地图</a>
								<em>© 2015-2025 Hengwang.com 版权所有. 更多模板 <a href="http://www.cssmoban.com/" target="_blank" title="模板之家">模板之家</a> - Collect from <a href="http://www.cssmoban.com/" title="网页模板" target="_blank">网页模板</a></em>
							</p>
						</div>
					</div>
	</body>

</html>
	<script>
	var mid = document.getElementById('but');
	var i = 10;
	setInterval(function fun() {

	console.log(i);	
	i--;
	if(i<=0){
		mid.innerHTML = '重新发送验证邮件';
		//alert(mid.disabled);
		mid.disabled = false;
	}else{
		mid.innerHTML = i;
	}
},500)
	</script>